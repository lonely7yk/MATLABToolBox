function [outclass,TabCan,TabL,TabCon,TabM,TabG,trainscore] = Fisher(training,group,sampledata,contri)
%FISHER 判别分析.
%   class = fisher(training,group,sampledata) 根据训练样本training构造判别式，
...
%   group具有相同的数据类型. 
%
%   class = fisher(training,group,sampledata,contri) 根据累积贡献率不低于
...
CumContGeCon = find(CumCont >= contri - 0.000000001);
