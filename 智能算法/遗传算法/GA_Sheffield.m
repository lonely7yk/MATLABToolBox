...
function [trace,noteExtrem,extremY,extremX,Population] = GA_Sheffield(nvars,lb,ub,command)
	% input
	% nvars = 2;		% 变量数量
	...
	% extremY : 最优 Y
	% extremX : 最优 Y 对应 X
	% Population : Population.init 为初始种群（二进制），Population.best 为最优种群（二进制）
...
%% Fitness: 适应度函数（注意这里的 X 可能是一个多维矩阵，所以不能只针对一个 X 来写，ObjV 必须是一个列向量）
